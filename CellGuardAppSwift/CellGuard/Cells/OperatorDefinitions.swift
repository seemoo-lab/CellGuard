...
struct OperatorDefinitions {
    ...
    let countries: [Int: NetworkCountry]
    ...
    private init(countries: [NetworkCountry], operators: [NetworkOperator]) {
        ...
        self.countries = Dictionary(uniqueKeysWithValues: countries.map( { (Int($0.mcc) ?? -1, $0) } ))
    }
    
    // TODO: Refactor API usages
    func translate(country: Int32, iso: Bool = false) -> String? {
        let country = countries[Int(country)]
        return iso ? country?.iso : country?.name
    }
    
    // TODO: Refactor API usages
    func translate(country: Int32, network: Int32, iso: Bool = false) -> (String?, String?) {
        let countryName = translate(country: country, iso: iso)
        let operatorName = networks[Int(country)]?[Int(network)]?.first?.operatorName
        
        return (countryName, operatorName)
    }
